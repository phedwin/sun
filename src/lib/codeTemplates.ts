// Code template generator for different languages based on question
 ...
 */
export function generateCodeTemplates(questionTitle: string, questionSlug: string): CodeTemplate {
...
function generateJavaScript(functionName: string, params: { type: string; name: string }[], questionTitle: string): string {
  const paramList = params.map(p => p.name).join(', ');
  const paramTypes = params.map(p => `${p.name}: ${mapTypeToJS(p.type)}`).join(', ');
...
function generatePython(functionName: string, params: { type: string; name: string }[], questionTitle: string): string {
  const paramList = params.map(p => `${p.name}: ${mapTypeToPython(p.type)}`).join(', ');
...
function generateJava(functionName: string, params: { type: string; name: string }[], questionTitle: string): string {
...
function generateCpp(functionName: string, params: { type: string; name: string }[], questionTitle: string): string {
...
function mapReturnTypeToJS(title: string): string {
  const lower = title.toLowerCase();
  if (lower.includes('sum') && !lower.includes('maximum') && !lower.includes('minimum')) return 'number[]';
  if (lower.includes('valid') || lower.includes('is ')) return 'boolean';
  if (lower.includes('count') || lower.includes('number') || lower.includes('length')) return 'number';
...
function mapReturnTypeToPython(title: string): string {
  const lower = title.toLowerCase();
  if (lower.includes('sum') && !lower.includes('maximum') && !lower.includes('minimum')) return 'List[int]';
  if (lower.includes('valid') || lower.includes('is ')) return 'bool';
  if (lower.includes('count') || lower.includes('number') || lower.includes('length')) return 'int';
...
function mapReturnTypeToJava(title: string): string {
  const lower = title.toLowerCase();
  if (lower.includes('sum') && !lower.includes('maximum') && !lower.includes('minimum')) return 'int[]';
  if (lower.includes('valid') || lower.includes('is ')) return 'boolean';
  if (lower.includes('count') || lower.includes('number') || lower.includes('length')) return 'int';
  if (lower.includes('string')) return 'String';
...
function mapReturnTypeToCpp(title: string): string {
  const lower = title.toLowerCase();
  if (lower.includes('sum') && !lower.includes('maximum') && !lower.includes('minimum')) return 'vector<int>';
  if (lower.includes('valid') || lower.includes('is ')) return 'bool';
  if (lower.includes('count') || lower.includes('number') || lower.includes('length')) return 'int';
  if (lower.includes('string')) return 'string';
